...
#if defined(NDEBUG)
#error "TRPC cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uTRPC) are considered zero fee (for relaying and mining)
...
const string strMessageMagic = "Trpc Signed Message:\n";
...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : xTRPC with serial %s is already in the block %d\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : xTRPC spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: xTRPC spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from TRPC
...
void RecalculateXIONSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from xTRPC supply
...
bool RecalculateTRPCSupply(int nHeightStart)
{
...
/** FornaxA: Trpc accepts PoS during PoW phase 
 *
 *   if (pindex->nHeight <= Params().LAST_POW_BLOCK() && block.IsProofOfStake())
 *       return state.DoS(100, error("ConnectBlock() : PoS period not active"),
 *           REJECT_INVALID, "PoS-early");
 */
/** cevap: Trpc accepts PoS during PoW phase, PoW Ends with block 1000, PoS starts with 455, testnet 74 
 *
 */ 
/** cevap: Trpc accepts PoS during PoW phase, PoW Ends with block 1000, PoS starts with 455, testnet 74 
 *
 ...
 */
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateTRPCSupply(Params().Zerocoin_StartHeight());
    }

    //Track xTRPC money supply in the block index
    if (!UpdateXIONSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new xTRPC supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s xTRPCSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert Trpc to xTRPC
...
/*    if (block.GetBlockTime() > Params().Zerocoin_StartTime()) {
        ...
        // TRPCTor - disable reject block as our blocks are in version 8 since block 1
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent xTRPC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of xTRPC serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d xTRPC mints and %d xTRPC spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // TRPC: We use certain sporks during IBD, so check to see if they are
