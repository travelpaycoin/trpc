...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered trpc address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_3_SWIFTTX_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 TRPC.").arg(GetSporkValue(SPORK_3_SWIFTTX_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal trpc:URI (trpc:XyZ...?message=example)
            {
...
static void NotifyxTRPCReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyxTRPCReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyxTRPCReset.connect(boost::bind(NotifyxTRPCReset, this));
}
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyxTRPCReset.disconnect(boost::bind(NotifyxTRPCReset, this));
}
