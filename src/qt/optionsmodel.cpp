...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::TRPC);
    ...
    if (!settings.contains("nAnonymizeTRPCAmount"))
        settings.setValue("nAnonymizeTRPCAmount", 1000);

    nAnonymizeTRPCAmount = settings.value("nAnonymizeTRPCAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeTRPCAmount"))
        SoftSetArg("-anonymizeionamount", settings.value("nAnonymizeTRPCAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in trpc.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTRPCAmount:
            return QVariant(nAnonymizeTRPCAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTRPCAmount:
            nAnonymizeTRPCAmount = value.toInt();
            settings.setValue("nAnonymizeTRPCAmount", nAnonymizeTRPCAmount);
            emit anonymizeTRPCAmountChanged(nAnonymizeTRPCAmount);
            break;
        case CoinControlFeatures:
