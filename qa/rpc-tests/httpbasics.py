...
class HTTPBasicsTest (BitcoinTestFramework):        
    ...
    def run_test(self):        
        ...
        conn = httplib.HTTRPConnection(url.hostname, url.port)
        conn.connect()
        ...
        conn = httplib.HTTRPConnection(urlNode1.hostname, urlNode1.port)
        conn.connect()
        conn.request('POST', '/', '{"method": "getbestblockhash"}', headers)
        out1 = conn.getresponse().read();
        assert_equal('"error":null' in out1, True)
        ...
        conn = httplib.HTTRPConnection(urlNode2.hostname, urlNode2.port)
        conn.connect()
        conn.request('POST', '/', '{"method": "getbestblockhash"}', headers)
        out1 = conn.getresponse().read();
        assert_equal('"error":null' in out1, True)
