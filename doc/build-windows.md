...
Below are some notes on how to build Trpc Core for Windows.

The options known to work for building Trpc Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the Trpc Core Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Trpc Core applications.
...
Note that for WSL the Trpc Core source path MUST be somewhere in the default mount file system, for
...
causes two of the trpc executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the trpc code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the trpc source code).
